...
exports.up = function(knex) {
    return knex.schema.createTable('recipe', tbl => {
        ...
        tbl.integer('recipe_id').unsigned().notNullable().references('id').inTable('recipe').onDelete('CASCADE').onUpdate('CASCADE');
        tbl.float('quantity').notNullable();
        tbl.integer('ingredient_id').unsigned().notNullable().references('id').inTable('ingredient').onDelete('CASCADE').onUpdate('CASCADE');
